...
np.random.seed(1)
X, T = quad_cylinder_mesh(R=0.22, H=0.22, h=0.02,f=1)

X[:,2] += 0.35; 
X += 0.00025*np.random.randn(X.shape[0],3) 
self = Cloth(X,T); 
dt = self.estimateTimeStep(L=2*np.pi*0.22)
self.plotMesh()
self.setSimulatorParameters(dt = dt, delta= 0.09, kappa= 0.2*1e-4, tol = 0.0075, str = 0.025*1e-4, shr = 15*1e-4, mu_s = 0.35, thck = 0.95, mu_f = 0.25)
